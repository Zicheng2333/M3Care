...
class PatientDataset(Dataset):
    def __init__(self, data, mappings, index_set, task):
        """
        初始化病人数据集
    ...
    def __getitem__(self, idx):
        ...
        if self.task in ['next_visit_diseases','readmission']:
            if len(visits)<2:
                return None
            target_visits = visits[:-1] #返回除最后一个元素之外的所有元素
            last_visit = visits[-2] #返回最后一个元素

            if self.task == 'next_visit_diseases':
                nxt = last_visit.get('next_visit_diseases', None)
                if nxt is None:
                    return None  # 或者跳过
                label = nxt

            elif self.task == 'readmission':
                label = last_visit.get('30_days_readmission', None)
                if label is None:
                    return None
            ...
            label = visits[-1].get('in_hospital_mortality', None)
            if label is None:
                return None
        else:
